...
**第 5 步 · 读结果**
- static(CLI / `deploy_commit` / `deploy_files`)→ **同步**返回,含新版本号 + 访问地址 `https://<name>.at.sowii.net`。若该 artifact 是 `anonymous`(匿名可访问)且检测合格,结果里还会多一行 **CDN 直连入口**(`https://m.sowii.net/sites/<name>/`,恒带尾斜杠;旧域名会 `301` 过去);不合格会逐条给出降级原因(常见:HTML/JS/CSS 里有根绝对路径引用、或含 `serviceWorker.register`),此时**仍按普通模式部署成功,不阻断**。⚠️ 该特性**上线状态=灰度中**——平台未开启时,即便站点本身合格,结果里也**不会**出现这行,**看不到不代表不合格**,不要据此误判 preflight/自查有问题。若 `cargo.yaml` 写了 `access.cdn: origin`(全代理,彻底关 CDN),结果里对应的提示行固定是:`access.cdn: origin——CDN 已关闭:全代理模式,全部请求经 cargo 域名流式回源(零重定向;字节经集群出口)`——不再是"退出直连"的旧措辞,也不会再出现 CDN 直连入口那一行。
