...
- `access.browser: key` 且该 artifact 此前无有效密钥 → **仅 CLI `picargo deploy` 会打印一次性访问密钥**,务必原样转告用户妥善保存;经远程 MCP 部署的 key 模式 artifact,结果里**不含**密钥——用 CLI `picargo access-key show --reveal` 取回(或 `access-key rotate` 换新)。
